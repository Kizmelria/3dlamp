    ...
    <div class="mt-4 mb bg-white px-2 py-2 mx-auto rounded-lg grid grid-cols-1 md:grid-cols-2" style="width: 80%">
        ...
        <div class="p-8 sm:pr-8 lg:pr-0 text-right relative" style="width:100%; height:100%;">
            <div class="relative">
                @if(!empty($product->category) && trim(strtolower($product->category)) !== 'none')
                    <span class="absolute top-0 left-0 rounded-md  bg-black p-3 text-center text-sm font-medium text-white">
                        {{ $product->category }}
                    </span>
                @endif
        ...
        <div class="p-8 space-y-1" style="width: 100%; height:100%;">
            ...
            <div class="space-y-4">
                ...
                <form action="{{ route('guest.checkout.process', $product->id) }}" method="POST">
                    @csrf
