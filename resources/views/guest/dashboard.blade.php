<x-app-layout>
    <div>
        <div class="bg-white dark:bg-gray-800 shadow-md flex flex-wrap items-center justify-between p-4 lg:px-24 space-y-4 lg:space-y-0">
            ...
            <div>
                <form class="flex items-center w-full lg:w-auto space-x-2" method="GET" action="{{ route('guest.dashboard') }}">
                    <div class="relative">
                    ...
                    <div class="inline-block relative">
                        ...
                        <div id="filterSection" class="hidden absolute right-0 top-full mt-2 bg-white rounded-lg shadow-xl border border-gray-200 p-4 md:p-6 z-50 w-[300px] sm:w-[600px] lg:w-[800px]">
                            ...
                            <div class="flex justify-end mt-6 space-x-2">
                                <a href="{{ route('guest.dashboard') }}" class="w-full sm:w-auto border border-gray-300 hover:bg-slate-100 text-black px-6 py-3 rounded-lg font-medium">Clear</a>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 px-4">
            ...
            <div class="bg-white rounded-xl shadow-lg overflow-hidden max-w-xs mx-auto relative" style="height: 500px; width:400px">
                ...
                <div class="absolute inset-0 flex items-center justify-center z-10 text-center">
                    <a href="{{ route('guest.product.view', $product->id) }}" class="absolute z-8 w-full h-full"></a>
                </div>
                @endif
                <div class="relative">
                    @if(isset($product->discount) && $product->discount > 0)
                        <span class="absolute top-0 left-0 w-28 translate-y-4 pr-2 -translate-x-6 -rotate-45 bg-black text-center text-sm text-white">
                            {{ $product->discount }}% Off
                        </span>
                    @endif
                    @if(!empty($product->category) && trim(strtolower($product->category)) !== 'none')
                        <span class="absolute top-0 right-0 m-2 rounded-full bg-black px-2 text-center text-sm font-medium text-white">
                            {{ $product->category }}
                        </span>
                    @endif
